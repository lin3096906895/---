...
export default function ProfileCard({ postCount, chatterCount, photoCount }: { postCount: number, chatterCount: number, photoCount: number }) {
  ...
  const getSocialHref = (type: string) => {
    const value = (social as Record<string, string | undefined>)[type] || "";
    if (!value) return "";
    if (type === "email") return `mailto:${value}`;
    if (type === "qq") {
      return /^\d+$/.test(value)
        ? `https://wpa.qq.com/msgrd?v=3&uin=${encodeURIComponent(value)}&site=qq&menu=yes`
        : value;
    }
    if (type === "wechat") {
      return value.startsWith("weixin://") ? value : `weixin://dl/chat?${encodeURIComponent(value)}`;
    }
    return value;
  };
  ...
  return (
    <div
      ...
      className="md:col-span-7 rounded-3xl bg-white/45 dark:bg-slate-800/50 backdrop-blur-md border border-white/35 dark:border-white/10 shadow-xl p-5 sm:p-6 md:p-8 flex flex-col justify-between transition-all duration-700 hover:scale-[1.01] cursor-pointer group relative overflow-hidden h-full min-h-[220px] md:min-h-[280px]"
    >
      <div className="flex items-start justify-between relative z-10">
        <div className="flex items-center gap-4 md:gap-6 w-full">
          <div className="w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-xl md:rounded-2xl bg-white/60 dark:bg-slate-700/60 p-1 shadow-lg flex-shrink-0 transition-transform duration-500 group-hover:rotate-3">
      ...
      <div className="flex flex-col md:flex-row items-center md:items-end justify-between mt-6 md:mt-8 gap-5 md:gap-6 relative z-10">
        ...
        <div className="flex gap-2 md:gap-3 flex-wrap justify-center md:justify-end w-full md:w-auto" onClick={(e) => e.stopPropagation()}>
          <SocialBtn type="github" href={getSocialHref("github")} />
          <SocialBtn type="gitee" href={getSocialHref("gitee")} />
          <SocialBtn type="google" href={getSocialHref("google")} />
          <SocialBtn type="email" href={getSocialHref("email")} onClick={!getSocialHref("email") ? () => copyToClipboard(social.email || '', '邮箱') : undefined} />
          <SocialBtn type="qq" href={getSocialHref("qq")} onClick={!getSocialHref("qq") ? () => copyToClipboard(social.qq || '', 'QQ号') : undefined} />
          <SocialBtn type="wechat" href={getSocialHref("wechat")} onClick={!getSocialHref("wechat") ? () => copyToClipboard(social.wechat || '', '微信号') : undefined} />
        </div>
      </div>
    </div>
  );
...
function SocialBtn({ type, href, onClick }: { type: string, href?: string, onClick?: () => void }) {
  const getIcon = () => {
    switch (type) {
  ...
  return (
    <a href={href} target={href.startsWith("mailto:") || href.startsWith("weixin://") ? "_self" : "_blank"} rel="noopener noreferrer">
